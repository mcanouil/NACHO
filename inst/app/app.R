  ...
  shiny::navbarPage(
        ...
        list(
          ...
          shiny::radioButtons(
            ...
            choiceNames = list(
              shiny::tags$span(
                "MAX/FLEX",
                shiny::helpText("(Default: 0.1 - 2.25)")
              ),
              shiny::tags$span(
                "SPRINT",
                shiny::helpText("(Default: 0.1 - 1.8)")
              )
            ),
    ...
    shiny::tabPanel(
      ...
      card(
        ...
        body = list(
          shiny::uiOutput("outliers-thresholds"),
          shiny::tableOutput("outliers")
        )
      )
    ),
    shiny::tabPanel(
...
server <- function(input, output, session) {
  ...
  output$rcc_contents_summary <- shiny::renderUI({
    ...
    shiny::tags$p(
      "A total of",
      shiny::tags$strong(length(input$rcc_files[, "name"])),
      "RCC files were successfully uploaded,",
      "for a total amount of",
      shiny::tags$strong(format(rcc_size, units = "Mb")),
      "."
    )
  })
    ...
    if (is.null(input$rcc_files)) {
      shiny::fluidRow(
        ...
        shiny::column(
          ...
          card(
            title = "Summary",
            body = shiny::tags$p("No RCC files uploaded.")
          )
        )
      )
    } else {
      list(
        shiny::fluidRow(
          ...
          shiny::column(
            ...
            card(
              title = "Summary",
              body = shiny::uiOutput("rcc_contents_summary")
            )
          )
        ),
        shiny::fluidRow(
          ...
          shiny::column(
            width = 12,
            card(
              title = "RCC Files Uploaded",
              body = shiny::tableOutput("rcc_contents")
            )
          )
        )
      )
  ...
  observe({
    if (inherits(nacho_object, "nacho")) {
      ...
      saveRDS(
        object = nacho_object,
        file = file.path(tempdir(), "nacho_object.rds")
      )
    }
  })
