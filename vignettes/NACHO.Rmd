...
#| out-width: 650px
#| fig-alt: >-
#|   NACHO dashboard on the QC Metrics tab, with threshold sliders on the left
#|   and boxplots of binding density, field of view, positive control linearity
#|   and limit of detection per cartridge.
...
`normalise()` returns a `list` object, like `load_rcc()`.
Its `nacho` slot holds the raw counts in `Count` and the normalised counts in `Count_Norm`.
...
#|   normalisation method and a file picker for RCC files.
